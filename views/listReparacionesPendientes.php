<div class="card text-center">
     ...
     <i class="bi bi-tools"></i> LISTA DE REPARACIONES PENDIENTES  <i class="bi bi-tools"></i>
     </div>
     <div class="card-body">
         <table id="myTableP" class=" table table-hover table-bordered table-lg table-responsive">
...
if ($filter) {
    ...
    $sql = mysqli_query($con, "SELECT * FROM report INNER JOIN proprieter ON report.codigo_propietario = proprieter.codigo INNER JOIN repairmen ON report.id_reparador=repairmen.identificacion WHERE EstadoReporte='PENDIENTE'"); 
} else {
    $sql = mysqli_query($con, "SELECT * FROM report INNER JOIN proprieter ON report.codigo_propietario = proprieter.codigo INNER JOIN repairmen ON report.id_reparador=repairmen.identificacion WHERE EstadoReporte='PENDIENTE'"); 
    ...
    while ($row = mysqli_fetch_assoc($sql)) {
        ...
        echo '
						  ...
						  <tr style="font-size:12px">
                            ...
                            <td><a href="printCertificaties.php?nik=' .$row['id'].'&code='.$row['codigo_propietario'].'&repairmen='.$row['id_reparador'].'"id="send" title="Imprimir certificado" class="btn btn-outline-success btn-sm"><span class="fa fa-print" aria-hidden="true"></span></a></td>
